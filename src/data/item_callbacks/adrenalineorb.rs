...
pub fn on_after_boost(
    ...
    // JS: if (target.boosts['spe'] === 6 || boost.atk === 0) return;
    //
    // boost is the post-TryBoost table: an Intimidate drop blocked by an
    // ability (Clear Body, Hyper Cutter, ...) DELETES the atk key, while a
    // drop capped at -6 leaves atk present with value 0. Only the latter
    // matches `boost.atk === 0` in JS; a DELETED entry is undefined there.
    ...
    // JS: if (effect.name === 'Intimidate') target.useItem();
